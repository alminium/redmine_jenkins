...
Jenkins is application for Continuous Integration.
...
Jenkins Build and Related Changeset are too much, so plugin limits number of Builds and Changesets per each Jenkins Job.
You can change limit settings on settings/plugin/redmine_hudson.

=== Rake task 'redmine_jenkins:fetch'

You can get build results using rake task 'redmine_jenkins:fetch'.
...
If you use old version of Jenkins, plugin may not get test results and changesets.
